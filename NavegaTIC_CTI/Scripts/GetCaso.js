...
function refreshubicacion(val, selected) {
    ...
    if ($("#Canal").val() == "CHAT - SAE") {
        var Ubicacion = "UbicacionSAE";
    } else {
...
function refreshsubcategoria(val, selected, cargo) {
    ...
    if ($("#Canal").val() == "CHAT-SAE" || $("#Canal").val() == "IVR-SAE") {
        var metodo = "SubcategoriaSAE";
    } else {
        var metodo = "Subcategoria";
    }
    $('#subcategoria').html("Cargando..");
    var validatecargo = cargo.indexOf("GERENTE");
    fetch("/Home/"+metodo+"?clave=" + val)
        .then(function (result) {
            if (result.ok) {
                return result.json();
            }
...
function valorsla() {
    ...
    $.ajax({
        ...
        error: function () {
            alert('Para cerrar el caso, debe diligenciar el Escalado A y Categoría');
        }
    });
    /*
...
function listcorreos(val) {
    if ($("#Canal").val() == "CHAT-SAE" || $("#Canal").val() == "IVR-SAE") {
        var metodo = "listcorreosSAE";
    } else {
        var metodo = "listcorreos";
    }
    var list = "";
    fetch("/Home/" + metodo +"?clave=" + val)
        .then(function (result) {
...
function deshabilitar_transferencia() {
    if ($("#Canal").val() == "CHAT-SAE" ) {
...
function llenarTranferidode(value) {
    if (value == 540) { //Canal transferencia
        
        if ($("#Canal").val() == "IVR" || $("#Canal").val() == "chat") {

            $("#transferidade").val("SAE");

        } else if ($("#Canal").val() == "IVR-SAE" || $("#Canal").val() == "CHAT-SAE") {

            $("#transferidade").val("Soporte");

        } 
    }
    else {

        $("#transferidade").val("N/A");
...
async function Guardar($Btn) {
    try {
        ...
        if (($("#subcategoria").val() == 36) && ($("#detalle2").val() == "")) {
            ...
            params["Id_LlamadaIVR"] = $("#Id_LlamadaIVR").val();
            params["Transferida"] = $("#transferencia").val();
            params["Transferidade"] = $("#transferidade").val();
...
function ToolsFuncBtnLoader($Btn, Ocultar) {
    ...
    function validarCaso() {
        ...
        var idCaso = document.getElementById("CasoId").value;

            if (idCaso !== "") {
                $.ajax({
                    type: "GET",
                    url: "/Home/ValidarCaso",
                    data: { idCaso: idCaso },
                    success: function (response) {
                        if (response === true) {
                            // El caso es válido
                            document.getElementById("botonGuardar").disabled = true;
                            alert("El caso, " + idCaso + " ya fue gestionado.");
                        } else {
                            // El caso no es válido
                            document.getElementById("botonGuardar").disabled = false;
                            //alert("El caso no es válido.");
                        }
                    },
                    error: function () {
                        alert("Error al validar el caso.");
                    }
                });
            } else {
               // alert("Ingrese un ID de caso válido.");
            }
    }
